...
col(col == 0) = nan; %replace all 0s with nan
...
g = nan*ones(numrow,numcol,2); %grid of circles
...
for ii = 1:numrow
    ...
    for jj = 1:numcol
        ...
        %no common elements (no circle in spot)
        if isempty(com)
            g(ii,jj,:) = [nan nan]; %null as placeholder
